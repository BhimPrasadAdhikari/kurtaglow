      ...
      <div className="relative ml-4 flex flex-1 flex-col justify-between sm:ml-6">
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center">
          ...
          <div className="flex items-center mt-2 sm:mt-0">
            <IconButton className="dark:bg-black mr-2" onClick={() => { cart.increaseItem(data.id, 'DECREASE') }} icon={<MinusIcon />} />
            <Card className="w-10 h-10 flex items-center justify-center">
              <CardContent>{item ? item.quantity : 1}</CardContent>
            </Card>
            <IconButton onClick={() => { cart.increaseItem(data.id, 'INCREASE') }} className="dark:bg-black ml-2" icon={<PlusIcon />} />
          </div>
