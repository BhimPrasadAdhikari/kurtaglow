...
const Summary: React.FC<SummaryProps> = ({ summaryData }) => {
  ...
  const productDetails = items.map((product) => ({
    identity: product.id,
    name: product.name,
    total_price: product.discount==0? product.price*product.quantity:(product.price-(product.discount*product.price)/100)*product.quantity,
    ...
    if (searchParams?.get("status") === "Completed") {
      const checkStatus = async () => {
        try {
          ...
          if (lookup_response.statusText === "OK") {
            toast.success(`Paymnet ${lookup_response.data.status} `);
            try {
              setLoading(true);
              const response = await axios.post(
                ...
                JSON.stringify({
                  ...
                  pidx: lookup_response.data.pidx,
                  total_amount: lookup_response.data.total_amount,
                  transaction_id: lookup_response.data.transaction_id,
              ...
              if (response.statusText === "OK") {
                window.location.href=`${response.data.url}/orders`
                cart.removeAll();
                toast("Success");
              }
            } catch (error) {
              ...
              console.error(
                "ERROR_CREATING_ORDER_OR_VERIFICATION_ERROR",
                error
              );
            }
          }
          ...
          console.error("Error fetching lookup khalti Response", error);
        }
      };
      checkStatus(); // Call the function only if pidx is not empty
    }
      ...
      >
        <div hidden>
          ...
          <input
            ...
            id="total_amount"
            name="total_amount"
            defaultValue={totalPrice+totalPrice * 0.13+120}
            required
          />
          <input
            type="text"
        ...
        >
          <Image
            src="https://rc-epay.esewa.com.np/assets/img/logo.svg"
            width={120}
            height={31}
            alt="esewa logo"
          />
